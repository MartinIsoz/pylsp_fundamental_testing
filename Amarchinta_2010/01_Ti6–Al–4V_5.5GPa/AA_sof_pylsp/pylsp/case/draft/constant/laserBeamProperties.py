...
{\n\
    ...
    startPoint = 'startPoint ( ' + str(laserBeam.startPoint.x) + \
        ' ' + str(laserBeam.startPoint.y) + \
        ' ' + str(laserBeam.startPoint.z) + ' );\n\n'
    endPoint = 'endPoint ( ' + str(laserBeam.endPoint.x) + \
        ' ' + str(laserBeam.endPoint.y) + \
        ' ' + str(laserBeam.endPoint.z) + ' );\n\n'

    timeProfileString = 'timeProfile ((0.0 1.0) (1e10 1.0));\n'
    ...
    if isinstance(laserBeam.spaceProfile, pylsp.PiecewiseLinearLaserSpaceProfile):
        ...
        for i in range(0, len(laserBeam.spaceProfile.radius)):
            spaceProfileString += '('
            spaceProfileString += str(laserBeam.spaceProfile.radius[i])
            spaceProfileString += ' '
            spaceProfileString += str(laserBeam.spaceProfile.mult[i])
            spaceProfileString += ') '
            i = i + 1
