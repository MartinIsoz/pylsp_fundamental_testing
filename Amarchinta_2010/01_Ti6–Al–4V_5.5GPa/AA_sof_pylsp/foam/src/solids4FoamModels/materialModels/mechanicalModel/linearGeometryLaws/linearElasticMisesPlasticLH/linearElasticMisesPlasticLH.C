...
#include "linearElasticMisesPlasticLH.H"
...
namespace Foam
{
    defineTypeNameAndDebug(linearElasticMisesPlasticLH, 0);
    addToRunTimeSelectionTable
    (
        mechanicalLaw, linearElasticMisesPlasticLH, linGeomMechLaw
    );

// * * * * * * * * * * * * * * Static Members  * * * * * * * * * * * * * * * //

    // Tolerance for Newton loop
    scalar linearElasticMisesPlasticLH::LoopTol_ = 1e-8;

    // Maximum number of iterations for Newton loop
    label linearElasticMisesPlasticLH::MaxNewtonIter_ = 200;

    // finiteDiff is the delta for finite difference differentiation
    scalar linearElasticMisesPlasticLH::finiteDiff_ = 0.25e-6;

    // Store sqrt(2/3) as we use it often
    scalar linearElasticMisesPlasticLH::sqrtTwoOverThree_ = ::sqrt(2.0/3.0);
...
void Foam::linearElasticMisesPlasticLH::updatePlasticity
(
...
Foam::scalar Foam::linearElasticMisesPlasticLH::curYieldStress
(
            scalar& sigmaYqs,
            scalar& sigmaYr,
...
) const
{
    ...
    //Info << "curYieldStress:" << endl;

    //Info << "curEpsilonPEq:" << curEpsilonPEq << endl;
    //Info << "oldEpsilonPEq:" << oldEpsilonPEq << endl;
    
    scalar deltaT = mesh().time().deltaT().value();
    //Info << "deltaT:" << deltaT << endl;

    scalar epsP = max(oldEpsilonPEq, curEpsilonPEq);

    scalar epsPdot = 0.0;
    if (deltaT > 0.0) {
        epsPdot = (epsP - oldEpsilonPEq) / deltaT;
    }

    sigmaYqs = K0_ * pow(epsP + eps0_, n_);

    scalar q = q1_ / pow(epsP + q2_, q3_);

    sigmaYr = (1.0 + q * pow(epsPdot, p_)) / (1.0 + q * pow(epsDot0_, p_));

    if (sigmaYr < 1.0) {
        sigmaYr = 1.0;
    }
   
    return sigmaYqs * sigmaYr;
}


Foam::scalar Foam::linearElasticMisesPlasticLH::yieldFunction
(
    scalar& sigmaYqs,
    scalar& sigmaYr,
    const scalar epsilonPEqOld,    // Old equivalent plastic strain
...
void Foam::linearElasticMisesPlasticLH::newtonLoop
(
...
) const
{
    ...
    int i = 0;
    // scalar deltaT = mesh().time().deltaT().value();   
    // DLambda = epsDot0_ * deltaT;
    // finiteDiff_ = epsDot0_ * deltaT;
    // scalar fTrial = yieldFunction(sigmaYqs, sigmaYr,epsilonPEqOld, magSTrial, DLambda, muBar);
    // if (fTrial < 0.0) {
    //     DLambda = 0.0;
    //     fTrial = yieldFunction(sigmaYqs, sigmaYr,epsilonPEqOld, magSTrial, DLambda, muBar);
    // }
    ...
    do
    {
        ...
        const scalar fTrialStep  =
            yieldFunction
            (
                sigmaYqs, sigmaYr, epsilonPEqOld, magSTrial, DLambda + finiteDiff_, muBar
            );
        ...
        if (i == MaxNewtonIter_)
        {
            WarningIn("linearElasticMisesPlasticLH::newtonLoop()")
...
void Foam::linearElasticMisesPlasticLH::calculateHydrostaticStress
(
...
)
{
    if (solvePressureEqn_)
    {
        ...
        else
        {
            FatalErrorIn
            (
                "void Foam::linearElasticMisesPlasticLH::"
...
Foam::linearElasticMisesPlasticLH::linearElasticMisesPlasticLH
(
    const word& name,
    const fvMesh& mesh,
...
:
    ...
    rho_(dict.getOrDefault<dimensionedScalar>("rho", 1.0)),
    ...
    q3_(readScalar(dict.lookup("q3"))),
    p_(readScalar(dict.lookup("p"))),
    n_(readScalar(dict.lookup("n"))),
    eps0_(readScalar(dict.lookup("eps0"))),
    epsDot0_(readScalar(dict.lookup("epsDot0"))),
    ...
    (
        ...
        dimensionedScalar
        (
            "initialYieldStress", dimPressure, K0_ * pow(eps0_, n_)
        )
    ),
    sigmaYqs_
    (
    ...
    (
        ...
        dimensionedScalar
        (
            "initialYieldStress", dimPressure, 1.0
        )
    ),
    sigmaYf_
    (
    ...
    (
        ...
        mesh,
        dimensionedSymmTensor("zero", Foam::dimensionSet(0, 0, -1, 0, 0, 0, 0), symmTensor::zero)
    ),
    epsilonPf_
    (
        IOobject
    ...
    (
        ...
        mesh,
        dimensionedScalar("zero", Foam::dimensionSet(0, 0, -1, 0, 0, 0, 0), 0.0)
    ),
    epsilonPEqf_
    (
        IOobject
    ...
    )
{
    ...
    epsilon_.oldTime();
    epsilonP_.oldTime();
    epsilonPdot_.oldTime();
    epsilonPf_.oldTime();
    epsilonPfDot_.oldTime();
    epsilonPEq_.oldTime();
    epsilonPEqDot_.oldTime();
    epsilonPEqf_.oldTime();
    epsilonPEqfDot_.oldTime();
    plasticN_.oldTime();
    sigmaY_.oldTime();
    sigmaYf_.oldTime();
    ...
    if (dict.found("E") && dict.found("nu"))
    {
        ...
        E_ = dimensionedScalar(dict.getOrDefault<dimensionedScalar>("E", 1.0));

        // Read the Poisson's ratio
        nu_ = dimensionedScalar(dict.getOrDefault<dimensionedScalar>("nu", 0.33));
    ...
    {
        // Read shear modulus
        mu_ = dimensionedScalar(dict.getOrDefault<dimensionedScalar>("mu", 1.0));

        // Read bulk modulus
        K_ = dimensionedScalar(dict.getOrDefault<dimensionedScalar>("rho", 1.0));
    ...
    else
    {
        FatalErrorIn
        (
            "linearElasticMisesPlasticLH::linearElasticMisesPlasticLH::()"
...
Foam::tmp<Foam::volScalarField> Foam::linearElasticMisesPlasticLH::rho() const
{
...
Foam::linearElasticMisesPlasticLH::impK() const
{
...
Foam::linearElasticMisesPlasticLH::impKdiagTensor() const
{
    // Calculate scaling factor to ensure optimal convergence
    // This is similar to the tangent matrix in FE procedures
...
void Foam::linearElasticMisesPlasticLH::correct(volSymmTensorField& sigma)
{
    ...
    forAll(fTrial.boundaryField(), patchI)
    {
        ...
        forAll(fTrialP, faceI)
        {
            ...
            (
                ...
                DSigmaYP[faceI],
                sigmaYP[faceI],
                sigmaYqs_.boundaryFieldRef()[patchI][faceI],
                sigmaYr_.boundaryFieldRef()[patchI][faceI],
                sigmaYOldP[faceI],
                fTrialP[faceI],
                sTrialP[faceI],
    ...
    epsilonPdot_ = max(epsilonPdot_.oldTime(), DEpsilonP_ / deltaT);
    epsilonPEqDot_ = max(epsilonPEqDot_.oldTime(), DEpsilonPEq_ / deltaT);
...
void Foam::linearElasticMisesPlasticLH::correct(surfaceSymmTensorField& sigma)
{
    // Calculate total strain
    if (incremental())
    {
    ...
    forAll(fTrial.boundaryField(), patchI)
    {
        ...
        forAll(fTrialP, faceI)
        {
            ...
            (
                ...
                DSigmaYP[faceI],
                sigmaYP[faceI],
                sigmaYqsf_.boundaryFieldRef()[patchI][faceI],
                sigmaYrf_.boundaryFieldRef()[patchI][faceI],
                sigmaYOldP[faceI],
                fTrialP[faceI],
                sTrialP[faceI],
                epsilonPEqOldP[faceI],
    ...
    dimensionedScalar deltaT = mesh().time().deltaT();
    epsilonPfDot_ = DEpsilonPf_ / deltaT;
    epsilonPEqfDot_ = DEpsilonPEqf_ / deltaT;
...
Foam::scalar Foam::linearElasticMisesPlasticLH::residual()
{
...
void Foam::linearElasticMisesPlasticLH::updateTotalFields()
{
...
Foam::scalar Foam::linearElasticMisesPlasticLH::newDeltaT()
{
...
const Foam::volScalarField& Foam::linearElasticMisesPlasticLH::getEpsilonPEq() const {
    return epsilonPEq_;
}
