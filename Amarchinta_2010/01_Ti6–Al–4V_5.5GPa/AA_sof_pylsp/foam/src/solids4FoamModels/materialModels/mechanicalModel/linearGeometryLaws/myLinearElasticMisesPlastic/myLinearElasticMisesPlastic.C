...
#include "myLinearElasticMisesPlastic.H"
...
namespace Foam
{
    defineTypeNameAndDebug(myLinearElasticMisesPlastic, 0);
    addToRunTimeSelectionTable
    (
        mechanicalLaw, myLinearElasticMisesPlastic, linGeomMechLaw
    );

// * * * * * * * * * * * * * * Static Members  * * * * * * * * * * * * * * * //

    // Tolerance for Newton loop
    scalar myLinearElasticMisesPlastic::LoopTol_ = 1e-8;

    // Maximum number of iterations for Newton loop
    label myLinearElasticMisesPlastic::MaxNewtonIter_ = 200;

    // finiteDiff is the delta for finite difference differentiation
    scalar myLinearElasticMisesPlastic::finiteDiff_ = 0.25e-6;

    // Store sqrt(2/3) as we use it often
    scalar myLinearElasticMisesPlastic::sqrtTwoOverThree_ = ::sqrt(2.0/3.0);
...
void Foam::myLinearElasticMisesPlastic::updatePlasticity
(
...
) const
{
    ...
    else
    {
        ...
        if (magS > SMALL)
        {
            ...
            Pout << "sigmaYOld" << sigmaYOld << endl;
            Pout << "fTrial" << fTrial << endl;
            Pout << "sTrial" << sTrial << endl;
            Pout << "epsilonPEqOld" << epsilonPEqOld << endl;
        }
        else
        {
...
Foam::scalar Foam::myLinearElasticMisesPlastic::curYieldStress
(
...
Foam::scalar Foam::myLinearElasticMisesPlastic::yieldFunction
(
    const scalar epsilonPEqOld,    // Old equivalent plastic strain
...
void Foam::myLinearElasticMisesPlastic::newtonLoop
(
...
) const
{
    ...
    do
    {
        ...
        if (i == MaxNewtonIter_)
        {
            WarningIn("myLinearElasticMisesPlastic::newtonLoop()")
...
void Foam::myLinearElasticMisesPlastic::calculateHydrostaticStress
(
...
)
{
    if (solvePressureEqn_)
    {
        ...
        else
        {
            FatalErrorIn
            (
                "void Foam::myLinearElasticMisesPlastic::"
...
Foam::myLinearElasticMisesPlastic::myLinearElasticMisesPlastic
(
    const word& name,
    const fvMesh& mesh,
    ...
    )
{
    ...
    else
    {
        FatalErrorIn
        (
            "myLinearElasticMisesPlastic::myLinearElasticMisesPlastic::()"
...
Foam::tmp<Foam::volScalarField> Foam::myLinearElasticMisesPlastic::rho() const
{
...
Foam::myLinearElasticMisesPlastic::impK() const
{
...
//- Auxiliary to be able to access cells that yielded
Foam::tmp<Foam::volScalarField>
Foam::myLinearElasticMisesPlastic::yielded() const
{
    return tmp<volScalarField>
    (
        new volScalarField
...
Foam::myLinearElasticMisesPlastic::impKdiagTensor() const
{
    // Calculate scaling factor to ensure optimal convergence
    // This is similar to the tangent matrix in FE procedures
...
void Foam::myLinearElasticMisesPlastic::correct(volSymmTensorField& sigma)
{
...
void Foam::myLinearElasticMisesPlastic::correct(surfaceSymmTensorField& sigma)
{
    // Calculate total strain
    if (incremental())
    {
...
Foam::scalar Foam::myLinearElasticMisesPlastic::residual()
{
...
void Foam::myLinearElasticMisesPlastic::updateTotalFields()
{
    ...
    forAll(activeYield_.primitiveField(), celli)
    {
        if (DEpsilonPEq_.primitiveField()[celli] > SMALL)
        {
            ...
            yielded_.primitiveFieldRef()[celli] = 1.0;
            numCellsYielding++;
        }
        else
        {
    ...
    forAll(activeYield_.internalField(), celli)
    {
        if (DEpsilonPEq_.internalField()[celli] > SMALL)
        {
            ...
            yielded_.internalField()[celli] = 1.0;
            numCellsYielding++;
        }
        else
        {
    ...
    yielded_.correctBoundaryConditions();
...
Foam::scalar Foam::myLinearElasticMisesPlastic::newDeltaT()
{
